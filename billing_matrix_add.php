...
if (empty($action) OR $action == "showform")
{
    ...
    echo show_form_errors('billing_matrix_add');
    clear_form_errors('billing_matrix_add');
    
    echo "<form name='billing_matrix_add' action='{$_SERVER['PHP_SELF']}' method='post'>";
    
    echo "<p align='center'>{$strTag}: <input type='text' name='tag' value='{$_SESSION['formdata']['billing_matrix_add']['tag']}' /></p>";
    ...
    while ($hour < 24)
    {
        ...
        foreach ($days AS $day)
        {
            ...
            if (!empty($_SESSION['formdata']['billing_matrix_add'][$id])) $i = $_SESSION['formdata']['billing_matrix_add'][$id];
            else $i = '';  
...
elseif ($action == "add")
{
    ...
    {
        $errors++;
        $_SESSION['formerrors']['billing_matrix_add']['tag'] = sprintf($strFieldMustNotBeBlank, $strTag);
    }
    ...
    {
        $errors++;
        $_SESSION['formerrors']['billing_matrix_add']['tag1'] = sprintf(strADuplicateAlreadyExists, $strTag);
    }    
    
    
    if ($errors >= 1)
    {
        $_SESSION['formdata']['billing_matrix_add'] = cleanvar($_POST, TRUE, FALSE, FALSE,
                                                     array("@"), array("'" => '"'));
